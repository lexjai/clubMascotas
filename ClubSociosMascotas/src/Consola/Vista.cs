    ...
    public class Vista
    {
        ...
        public int[] TryObtenerArrayInt(string prompt, int size, char separador = ',')
        {
            ...
            while (true)
            {
                ...
                if (input.ToLower().Trim() == CANCELINPUT) throw new Exception("Entrada cancelada ");
                try
                {
                ...
                {
                    if (input != "")
                        Mostrar($"Error: '{input}' ingrese una opción valida", ConsoleColor.DarkRed);
                }
            }
        }
        public DateTime TryObtenerFecha(string prompt)
        {
            var promptF = prompt.Trim() + " (d/m/a): ";
            while (true)
            {
                ...
                {
                    Mostrar($"Error: '{input[0]}/{input[1]}/{input[2]}' ingrese una fecha valida", ConsoleColor.DarkRed);
                }
            }
        }   
    }
